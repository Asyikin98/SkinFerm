...
def crawl_url(pageUrl, cleansercom_arr):
    url = 'https://www.ulta.com/skin-care-cleansers-face-wash?N=vr4yvtZ1z12qckZ1pqzbcjZ27gsZ1z13p3k'
    page = get(url)
    ...
    try:
        for cleansers in cleanser :
            ...
            first_product_name = cleansers.find('h4', class_='prod-title').get_text().strip()
            first_product_price = cleansers.find('span', class_='regPrice').get_text().strip()
            first_product_rating = cleansers.find('label', class_='sr-only').get_text().strip()
