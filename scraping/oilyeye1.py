...
sql = """INSERT INTO oilyeye (image, name, price, rating) VALUES (%s, %s, %s, %s)"""


def crawl_url(pageUrl, eye_arr):
    url = 'https://www.skinstore.com/skin-care/skincare-concern/oily-skin.list?pageNumber=1&facetFilters=averageReviewScore_auto_content:%5B4+TO+5%5D|en_skincareproducttype_content:Eye+Care'
    page = get(url)
    ...
    try:
        for eyes in eye :
            ...
            eye_arr.append((first_product_image, first_product_name, first_product_price, first_product_rating))
...
eye_arr = crawl_url("", [])
print(len(eye_arr))
cursor.executemany(sql, eye_arr)
conn.commit()
cursor.close()
conn.close()
